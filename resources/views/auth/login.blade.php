...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Login - ContentImpact CMS</title>
    ...
    <body class="min-h-screen flex items-center justify-center p-4">
        ...
        <div class="w-full max-w-md bg-[#192239]/60 border border-slate-800 rounded-3xl p-8 sm:p-10 shadow-2xl backdrop-blur-md">
            ...
            <div class="text-center mb-8 flex flex-col items-center">
                ...
                <p class="text-xs text-slate-400 font-medium">CMS for the Future</p>
            </div>
            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-6">
                ...
                <!-- Email/Username -->
                <div>
                    <label for="email" class="block text-xs font-semibold text-slate-400 mb-2">Username</label>
                    <div class="relative">
                        ...
                        <input 
                            ...
                            placeholder="jurnalis atau editor"
                        >
                    </div>
                </div>
                ...
                <div class="flex items-center justify-between text-xs">
                    <label class="flex items-center space-x-2 text-slate-450 cursor-pointer select-none">
                        <input type="checkbox" name="remember" class="rounded border-slate-800 bg-[#12192a] text-indigo-650 focus:ring-0 focus:ring-offset-0">
                        <span>Ingat saya</span>
                    </label>
                </div>

                <!-- Sign In Button -->
                <button type="submit" class="w-full bg-indigo-600 hover:bg-indigo-750 text-white font-bold text-sm py-4 rounded-2xl transition shadow-lg shadow-indigo-600/10">
                    Sign In
                </button>
                ...
                <!-- Guest CTA -->
                <a href="{{ route('home') }}" class="w-full bg-[#27324c] hover:bg-[#2c3957] text-slate-200 hover:text-white font-semibold text-sm py-4 rounded-2xl transition flex items-center justify-center space-x-2 text-center">
                    <span>Mulai sebagai Guest (Baca Berita)</span>
                </a>

                <!-- Register Link -->
                <div class="text-center pt-2">
                    <span class="text-xs text-slate-500">Belum punya akun? </span>
                    <a href="{{ route('register') }}" class="text-xs font-bold text-indigo-400 hover:text-indigo-300 hover:underline transition">Daftar sebagai Jurnalis</a>
                </div>
            </form>
        </div>
